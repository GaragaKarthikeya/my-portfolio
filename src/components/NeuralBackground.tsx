...
const useNeuralAnimation = (canvasRef: React.RefObject<HTMLCanvasElement | null>) => {
  ...
  useEffect(() => {
    ...
    // Use fewer nodes on mobile (threshold: 768px)
    const isMobileDevice = dimensions.width < 768;
    const maxNodeCount = isMobileDevice ? 50 : 100;

    if (nodes.current.length === 0) {
      const nodeCount = Math.min(Math.floor((dimensions.width * dimensions.height) / 15000), maxNodeCount);
    ...
    const gradientStart = isDark ? "rgba(180, 100, 255, 0.3)" : "rgba(124, 58, 237, 0.2)";
      ...
      nodes.current.forEach((currentNode, i) => {
        if (mouseActive.current) {
          ...
          if (dist < maxDist) {
            ...
            const grad = ctx.createLinearGradient(currentNode.x, currentNode.y, mousePos.x, mousePos.y);
        ...
        const isNearMouse =
          mouseActive.current &&
          Math.hypot(mousePos.x - currentNode.x, mousePos.y - currentNode.y) < 100;

        if (isNearMouse) {
          ctx.beginPath();
          ctx.arc(currentNode.x, currentNode.y, currentNode.radius * pulseFactor * 3, 0, Math.PI * 2);
        ...
        for (let j = i + 1; j < nodes.current.length; j++) {
          ...
          if (distance < 150) {
            ...
            const otherNearMouse =
              mouseActive.current &&
              Math.hypot(mousePos.x - otherNode.x, mousePos.y - otherNode.y) < 100;
            ctx.strokeStyle =
